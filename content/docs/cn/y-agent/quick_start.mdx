...
command=|<|send_message(receiver="裁判",message="填写消息的内容") |>|
  ```
    ...
    <Step>
    运行一下看看效果：
    <Tabs items={['Round-1 裁判', '1.A', '1.B','Round-2 裁判', '2.B','Round-3 裁判', '3.A', 'Round-4裁判', '4.A', '4.B', 'Round-5裁判']}>
  ...
  <Tab value="Round-4 裁判">![](./work_flow/imgs/demo2-3-5j.png)</Tab>
</Tabs>
    </Step>
    </Steps>
  </Accordion>
  ...
  <Accordion title="课外作业">
    <Steps>
    <Step>
    1、不使用聊天室消息的话，是否可以通过工作空间变量，完成这个游戏？
    2、去掉裁判节点，让玩家自己进行游戏，如何实现？
    </Step>
    </Steps>
...
[执行器](/docs/y-agent/work_flow/wf_nodes/executor_node). [批处理](/docs/y-agent/work_flow/wf_nodes/batch_node).
...
高级知识：创建自动化流程分析工具、自动添加测试和训练语料、自动添加文档知识库 TODO：
