...
terminate 工具指令：command=|<|terminate(message="回答用户问题,注意要友好简洁，通俗易懂")|>|
  ...
  <Accordion title="1.4 不想在提示词里使用 `notify` 或者 `terminate` ？">
  <Steps>
    <Step>
...
<Accordions>
  <Accordion title="接下来一步一步实现" id="gamedemo" >
    ...
    <Step>
    由于出现了循环连接，为了防止死循环，需要设置运行次数限制：（仅需要裁判节点即可）
