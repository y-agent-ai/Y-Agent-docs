...
|     | .工具名称 |     | 工具返回值 | .工具名称 |
| get_roles_outputs() |     |     | 同时取多个角色的输出，并合并 | .get_roles_outputs(\[role1,role2...\],field_key) |
...
| 以下属于代码块提示 |     | 提示 | 插入内容 |
| --- | --- | --- | --- |
| send_message |     | 向聊天室发送消息 | command=\|\<\|notify(receiver="填写角色名称",message="填写消息的内容") \|\>\| |
...
| batch |     | batch代码块，批处理使用的内部临时变量，batch存储的是，当前批次的数据集 | \{\{batch}} |
|     | .index | 当前批次的序号 | index |
...
- terminate:给用户发送结束消息的函数，发送后流程不在继续执行下游节点：
